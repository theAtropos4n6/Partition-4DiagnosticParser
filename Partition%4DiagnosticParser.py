...
def fullparse():
	...
	try: #checkarw an einai legit evtx log
		...
		first_key = list(FullParseRecordsDict.keys())[0]
		last_key = list(FullParseRecordsDict.keys())[-1]

		FullParseLogStartTime = FullParseRecordsDict[first_key]['Event']['System']['TimeCreated']['#attributes']['SystemTime'].replace('T', ' ').replace('Z', ' UTC')
		FullParseLogEndTime = FullParseRecordsDict[last_key]['Event']['System']['TimeCreated']['#attributes']['SystemTime'].replace('T', ' ').replace('Z', ' UTC')
